...
export function recordMetric(name: string, value: number, attributes: Record<string, string> = {}): void { safeRun(() => { if (config && Number.isFinite(value)) enqueue("metrics", metricPayload(config.applicationName, name, value, attributes)); }, config?.debugTrace); }
