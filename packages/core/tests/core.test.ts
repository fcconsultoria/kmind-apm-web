...
describe("kmind-apm-web core", () => {
  ...
  it("uses applicationName while accepting the legacy serviceName alias", () => { expect(resolveConfig({ applicationName: "portal", clientKey: "key" }).applicationName).toBe("portal"); expect(resolveConfig({ serviceName: "legacy-portal", clientKey: "key" }).applicationName).toBe("legacy-portal"); expect(resolveConfig({ applicationName: "portal", serviceName: "legacy-portal", clientKey: "key" }).applicationName).toBe("portal"); });
